...
GITHUB_REPO = "Khurshed11/trufel-bot-web"
...
def upload_file_to_github(file_path_in_repo, file_bytes, commit_message):
    """Загружает или обновляет файл в репозитории GitHub через API"""
    ...
    # Проверяем, существует ли файл (нужен sha для перезаписи)
...
def sync_products_json_to_github():
    """Считывает все товары из SQLite БД и синхронизирует products.json на GitHub"""
    try:
        ...
        for r in rows:
            products_list.append({
                ...
                "price": float(r[3]),
                "image_url": r[4],
                "category": r[5] if r[5] else "🧁 Трайфлы",
            })

        json_bytes = json.dumps(
            products_list, ensure_ascii=False, indent=4
        ).encode("utf-8")
        upload_file_to_github(
            "products.json",
            json_bytes,
            "Авто-синхронизация products.json из бота",
        )
    except Exception as e:
        print(f"Ошибка синхронизации JSON с GitHub: {e}")
...
class AdminStates(StatesGroup):
    ...
    waiting_for_new_cat_value = State()
...
def start_command(message):
    ...
    welcome_text = (
        ...
        "Выбирай вкусняшки на нашей витрине ниже. Ждем твоего заказа! 👇"
        + admin_text
    )
    bot.send_message(
        message.chat.id,
...
def show_cart(message):
    ...
    markup.row(
        types.InlineKeyboardButton(
            "❌ Очистить", callback_data="clear_cart"
        ),
...
def handle_clear_cart(call):
    ...
    bot.edit_message_text(
        "Ой, корзинка опустела... Ждем новых вкусностей! 🧺",
...
def start_checkout(call):
    ...
    bot.set_state(
        call.from_user.id,
        OrderStates.waiting_for_delivery_type,
        call.message.chat.id,
    )
    ...
    bot.send_message(
        call.message.chat.id,
        "Как бы вы хотели получить ваши вкусняшки? ✨",
        reply_markup=markup,
    )
...
def process_delivery_type(message):
    if message.text not in ["🚗 Доставка", "🏃 Самовывоз"]:
        bot.send_message(
            message.chat.id,
            "Пожалуйста, выберите один из вариантов на кнопочках ниже.",
        )
        return
    ...
    if message.text == "🚗 Доставка":
        ...
        bot.send_message(
            message.chat.id,
            "Напишите, пожалуйста, ваш адрес  (улица, дом, ориентир), куда привезти сладости: 🌸",
        ...
        bot.set_state(
            message.from_user.id, OrderStates.waiting_for_phone, message.chat.id
        )
        ...
        bot.send_message(
            message.chat.id,
            "Поделитесь, пожалуйста, вашим номером телефона, чтобы мы связались с вами:",
            reply_markup=markup,
        )
...
def process_address(message):
    ...
    bot.send_message(
        message.chat.id,
        "Поделитесь, пожалуйста, вашим номером телефона, чтобы мы держали связь: 💞",
        reply_markup=markup,
    )


@bot.message_handler(
    state=OrderStates.waiting_for_phone, content_types=["text", "contact"]
)
def process_phone(message):
    ...
    bot.set_state(
        message.from_user.id, OrderStates.waiting_for_name, message.chat.id
    )
    bot.send_message(
        message.chat.id,
        "Как нам называть вас при общении? 😊 (Ваше имя):",
        reply_markup=types.ReplyKeyboardRemove(),
    )
...
def process_name(message):
    ...
    bot.set_state(
        message.from_user.id, OrderStates.waiting_for_comment, message.chat.id
    )
    bot.send_message(
        message.chat.id,
        "Оставьте теплое пожелание или важный комментарий к заказу (например: 'без клубники' или 'сделайте надпись С ДР'). Если нет комментариев — напишите 'нет':",
    )
...
@bot.message_handler(
    func=lambda m: m.text == "📋 Управление меню"
...
def admin_manage_menu(message):
    ...
    if not products:
        bot.send_message(
            message.chat.id, "📭 В меню пока нет ни одного десерта."
        )
        return

    bot.send_message(
        message.chat.id,
        "👇 Список ваших товаров. Выберите нужное действие:",
    )
    ...
    for prod in products:
        ...
        btn_edit = types.InlineKeyboardButton(
            text="✏️ Изменить", callback_data=f"editselect_{prod_id}"
        )
        btn_delete = types.InlineKeyboardButton(
            text="❌ Удалить", callback_data=f"del_{prod_id}"
        )
        keyboard.add(btn_edit, btn_delete)

        bot.send_message(
            message.chat.id, text, parse_mode="Markdown", reply_markup=keyboard
        )
...
def callback_delete_product(call):
    ...
    try:
        bot.edit_message_text(
            ...
            text="❌ Этот десерт был полностью удален из базы и витрины.",
        )
    except Exception:
        bot.send_message(
            call.message.chat.id, "❌ Товар удален, но сообщение не удалось обновить."
        )


@bot.callback_query_handler(
    func=lambda call: call.data.startswith("editselect_")
)
def callback_edit_select_field(call):
    ...
    keyboard.add(
        types.InlineKeyboardButton(
            "📝 Название", callback_data=f"field_name_{product_id}"
        ),
        types.InlineKeyboardButton(
            "💰 Цена", callback_data=f"field_price_{product_id}"
        ),
        types.InlineKeyboardButton(
            "📄 Описание", callback_data=f"field_description_{product_id}"
        ),
        types.InlineKeyboardButton(
            "🖼 Фотографию", callback_data=f"field_image_id_{product_id}"
        ),
    )
...
def callback_field_chosen(call):
    ...
    field_titles = {
        ...
        "image_id": "отправьте новое фото десерта",
    }

    bot.set_state(
        call.from_user.id, AdminStates.waiting_for_new_value, call.message.chat.id
    )
    ...
    bot.send_message(
        call.message.chat.id,
        f"✍️ Введите {field_titles.get(field_name, 'данные')}:",
    )
    bot.answer_callback_query(call.id)


@bot.message_handler(
    state=AdminStates.waiting_for_new_value, content_types=["text", "photo"]
)
def process_field_update(message):
    ...
    if field_name == "image_id":
        if message.content_type == "photo":
            try:
                msg = bot.send_message(message.chat.id, "⏳ Сохраняем новое фото на GitHub...")
                file_info = bot.get_file(message.photo[-1].file_id)
                photo_bytes = bot.download_file(file_info.file_path)

                filename = f"photo_{int(time.time())}.jpg"
                github_image_path = f"images/{filename}"

                # Загружаем фото на GitHub
                upload_file_to_github(
                    github_image_path,
                    photo_bytes,
                    f"Обновлено фото для товара ID {product_id}",
                )
                new_value = github_image_path
                bot.delete_message(message.chat.id, msg.message_id)
            except Exception as e:
                bot.send_message(
                    message.chat.id,
                    f"❌ Ошибка обработки фото: {e}. Попробуйте ещё раз:",
                )
                return
        else:
        ...
        if message.content_type != "text":
            bot.send_message(
                message.chat.id, "❌ Пожалуйста, отправьте текстовое значение:"
            )
            return
        new_value = message.text.strip()
        ...
        if field_name == "price":
            try:
                ...
                bot.send_message(
                    message.chat.id,
                    "❌ Цена должна состоять только из цифр. Попробуйте снова:",
                )
                return
    ...
    bot.send_message(
        message.chat.id,
        "✅ Изменения применены! Витрина успешно обновлена автоматически. 🚀",
    )


# --- УПРАВЛЕНИЕ КАТЕГОРИЯМИ ---
@bot.message_handler(
    func=lambda m: m.text == "📁 Управление категориями"
    and m.from_user.id in config.ADMIN_IDS
)
def admin_manage_categories(message):
    ...
    if not categories:
        bot.send_message(
            message.chat.id, "📭 В базе данных пока нет ни одной категории."
        )
        return

    bot.send_message(
        message.chat.id,
        "📂 Список ваших категорий меню. При удалении категории удалятся и входящие в неё товары!",
    )
    ...
    for cat_id, cat_name in categories:
        ...
        keyboard = types.InlineKeyboardMarkup(row_width=2)
        btn_edit = types.InlineKeyboardButton(
            text="✏️ Переименовать", callback_data=f"catedit_{cat_id}"
        )
        btn_delete = types.InlineKeyboardButton(
            text="❌ Удалить", callback_data=f"catdel_{cat_id}"
        )
        keyboard.add(btn_edit, btn_delete)
...
def callback_edit_category_start(call):
    ...
    bot.set_state(
        call.from_user.id,
        AdminStates.waiting_for_new_cat_value,
        call.message.chat.id,
    )
    ...
    bot.send_message(
        call.message.chat.id,
        "✍️ Напишите новое название для этой категории (вместе с эмодзи):",
...
def process_category_rename(message):
    ...
    bot.send_message(
        message.chat.id,
        f"✅ Категория успешно переименована в *{new_name}*! Витрина обновится сама.",
        reply_markup=markup,
        parse_mode="Markdown",
    )
...
@bot.message_handler(
    func=lambda m: m.text == "➕ Добавить вкусняшку"
    and m.from_user.id in config.ADMIN_IDS
)
def admin_add_product_start(message):
    bot.delete_state(message.from_user.id, message.chat.id)
    bot.set_state(
        message.from_user.id, AdminStates.choosing_category, message.chat.id
    )
    ...
    markup.add("🔙 Назад в админку")
    bot.send_message(
        message.chat.id, "Куда добавим новый шедевр?", reply_markup=markup
    )
...
def admin_choose_category(message):
    ...
    try:
        ...
        bot.send_message(
            message.chat.id,
            "❌ Пожалуйста, выберите категорию кнопкой на экране!",
        )
        return
    ...
    bot.set_state(
        message.from_user.id, AdminStates.waiting_for_name, message.chat.id
    )
    bot.send_message(
        message.chat.id,
        "Напишите милое название десерта:",
        reply_markup=admin_back_keyboard(),
    )
...
def admin_input_name(message):
    ...
    if message.text.startswith("/") or (
        len(message.text) > 2 and message.text[1] == "."
    ):
        bot.send_message(
            message.chat.id,
            "⚠️ Пожалуйста, напишите текстовое название десерта вручную:",
            reply_markup=admin_back_keyboard(),
        )
        return
    ...
    bot.set_state(
        message.from_user.id, AdminStates.waiting_for_photo, message.chat.id
    )
    bot.send_message(
        message.chat.id,
        "Отправьте фотографию десерта:",
        reply_markup=admin_back_keyboard(),
    )


@bot.message_handler(
    state=AdminStates.waiting_for_photo, content_types=["text", "photo"]
)
def admin_input_photo(message):
    ...
    if message.content_type == "photo":
        try:
            msg = bot.send_message(message.chat.id, "⏳ Сохраняем фото на GitHub...")
            file_info = bot.get_file(message.photo[-1].file_id)
            photo_bytes = bot.download_file(file_info.file_path)
            ...
            # Загружаем фото прямо в репозиторий GitHub
            upload_file_to_github(
                github_image_path,
                photo_bytes,
                f"Добавлено новое фото {filename}",
            )
            image_url = github_image_path
            bot.delete_message(message.chat.id, msg.message_id)
        except Exception as e:
            bot.send_message(
                message.chat.id,
                f"❌ Не удалось обработать фото: {e}. Отправьте еще раз.",
                reply_markup=admin_back_keyboard(),
            )
            return
    else:
    ...
    bot.set_state(
        message.from_user.id, AdminStates.waiting_for_desc, message.chat.id
    )
    bot.send_message(
        message.chat.id,
        "Напишите состав, вес и аллергены:",
        reply_markup=admin_back_keyboard(),
    )
...
def admin_input_desc(message):
    ...
    bot.set_state(
        message.from_user.id, AdminStates.waiting_for_price, message.chat.id
    )
    bot.send_message(
        message.chat.id,
        "Укажите цену в сомони (только цифры):",
        reply_markup=admin_back_keyboard(),
    )
...
def admin_input_price(message):
    ...
    try:
        ...
        bot.send_message(
            message.chat.id,
            "Ой, введите цену только цифрами (например: 35):",
            reply_markup=admin_back_keyboard(),
        )
        return
    ...
    # Добавляем в SQLite БД
    db.add_product(cat_id, name, desc, price, photo_url)
    # Автоматически синхронизируем products.json на GitHub
    sync_products_json_to_github()
    ...
    bot.send_message(
        message.chat.id,
        f"🎉 Сладость '{name}' успешно добавлена в меню по цене {price} сомони! Фото сохранено навсегда! ✨",
        reply_markup=markup,
    )
...
@bot.message_handler(
    func=lambda m: m.text == "📁 Добавить категорию"
    and m.from_user.id in config.ADMIN_IDS
)
def admin_add_category_start(message):
    ...
    bot.set_state(
        message.from_user.id,
        AdminStates.waiting_for_category_name,
        message.chat.id,
    )
    bot.send_message(
        message.chat.id,
        "✍️ Напишите название для новой категории (например: 🍩 Пончики):",
        reply_markup=admin_back_keyboard(),
    )
...
def admin_input_category_name(message):
    ...
    if category_name.startswith("/"):
        bot.send_message(
            message.chat.id,
            "❌ Название категории не должно начинаться с '/'",
            reply_markup=admin_back_keyboard(),
        )
        return
    ...
    if success:
        ...
        bot.send_message(
            message.chat.id,
            f"⚠️ Категория с названием '{category_name}' уже существует!",
            reply_markup=markup,
        )
...
@bot.message_handler(
    state="*",
    func=lambda m: m.text == "🔙 Назад в админку"
    and m.from_user.id in config.ADMIN_IDS,
)
def global_admin_back(message):
    ...
    if current_state == AdminStates.waiting_for_name.name:
        ...
        bot.send_message(
            message.chat.id,
            "🔙 Выберите категорию для товара заново:",
            reply_markup=markup,
        )
        ...
        bot.send_message(
            message.chat.id,
            "🔙 Изменяем название десерта. Введите имя:",
            reply_markup=admin_back_keyboard(),
        )
        ...
        bot.send_message(
            message.chat.id,
            "🔙 Перепишите состав и описание десерта:",
            reply_markup=admin_back_keyboard(),
        )
...
def handle_web_app_data(message):
    try:
        ...
        current_cart["total_price"] = sum(
            item["cost"] for item in current_cart["items"]
        )
        ...
        bot.send_message(
            message.chat.id, f"Ошибка при разборе данных витрины: {e}"
        )
